...
**Multi-mesh draw.** The scene secondary issues a **single `vkCmdDrawIndexedIndirect`** over the per-group args buffer (`drawCount == number of distinct meshes`) rather than one call per group — distinct `.obj` paths become distinct slots → distinct groups → one fanned-out multi-draw over the shared mega buffers. Per-group `first_instance` (prefix sum) and `instance_count` are computed CPU-side in `sort_topology_by_mesh` on topology change.

**Next:** GPU-driven culling — a compute pass over the `GPURenderers` buffer that frustum-tests each instance and writes the per-group visible `instance_count` (and compacted MVPs), replacing the CPU topology derivation. The per-group bases (`first_instance`) stay CPU-maintained (from `sort_topology_by_mesh`), so **no GPU prefix sum is needed** — the cull only overwrites `instance_count` and fills a visible prefix of each group's MVP region.
