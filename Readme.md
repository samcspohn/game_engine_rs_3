...
The editor opens the test-game project by default (`--project crates/test-game`) and shows the same animated cube in its viewport, animated by an editor-side `Spinner` component until project-scene deserialisation lands. It also draws a retained-mode UI overlay ([ADR-0006](docs/ADR-0006-retained-mode-ui.md) phases 1–2) — a rounded, bordered panel with a live readout, an ASCII specimen and a grid swatch strip, all laid out by taffy rather than hand-positioned — which exists to make the upload behaviour observable: **F6** toggles it, and `ENGINE_UI_TRACE=1` prints a line only on frames where the UI uploaded anything at all.
