...
| GPU transform pipeline | A [`WorldTransformGpu`](crates/engine-render/src/transform_gpu.rs) owns the **device-local SoT** ("source of truth") buffers — one per component (`vec4` per entity slot for position / rotation / scale, sized to `entity_capacity`, grown geometrically) — plus the two compute pipelines. Per frame, in this order, all inside the slot's pre-recorded primary CB: (1) **scatter** ×3 (one dispatch per component) reads `staging_<comp>[i]` + `dirty_<comp>[i]` and writes `sot_<comp>[i]` iff bit `i` is set; (2) **mvp_build** reads SoT pos/rot/scale, indexed via the per-camera `instance → entity` lookup, multiplies by the per-frame `view_proj` and writes the per-camera MVP buffer; (3) the graphics scene secondary draws using that MVP buffer. The SoT promotion is what makes the device-side transforms readable by future passes (culling, particle emitters, multiple cameras' MVP builds) for free — PCIe traffic stays bounded by what actually changed. **Dirty-only sparse upload is live:** each frame drains `TransformHierarchy::Dirty`'s three per-component `AtomicU32` bitmasks (atomic `swap(0, Relaxed)`) and ORs the harvested words into every in-flight FrameSlot's CPU-side pending mask; the current slot then copies its pending mask into the mapped `staging_dirty_{pos,rot,scl}` buffer, walks the set bits to write *only those entities'* local TRS into `staging_{pos,rot,scl}` (raw lock-free SoA reads via `TransformHierarchy::{positions,rotations,scales}_raw()`), and clears its pending mask. Per-component masks mean a pure-rotation frame writes zero pos/scale data on either CPU or GPU. The SoT currently stores **local** TRS — `mvp_build_cs` composes the model matrix directly without parent-chain composition; multi-level hierarchies await a GPU-side global-composition pass. |
...
| Reusable command buffers | One `MultipleSubmit` primary command buffer **per swapchain image** ("FrameSlot"), composed from **four** `MultipleSubmit` **secondary** command buffers stratified by invalidation domain: a **scatter compute secondary** (lives on `FrameSlot`; three back-to-back compute dispatches — one per component — sharing the scatter pipeline), an **mvp-build compute secondary** (lives on `FrameSlot`; one dispatch over the camera's draw count, binds the camera's mvp-build set 0 + the FrameSlot's mvp-build set 1 with `view_proj`), a **scene secondary** (lives on `RenderCamera`; inherits the primary's dynamic-rendering scope; viewport + pipeline/descriptor binds + **one `draw_indexed_indirect` per distinct mesh** (ADR-0004 Phase 1; `instance_count` and `first_instance` baked into each `DrawIndexedIndirectCommand` drive HW instancing for that mesh's contiguous MVP-buffer slice)) and a **blit secondary** (lives on `FrameSlot`; offscreen color → swapchain image `vkCmdBlitImage`). The primary executes them in order — `execute(scatter)` → `execute(mvp_build)` → `begin_rendering(...)` → `execute(scene)` → `end_rendering` → `execute(blit)` — and vulkano's auto-sync infers all the cross-secondary barriers (`SHADER_WRITE→SHADER_READ` between scatter and mvp-build on the SoT buffers; `SHADER_WRITE→SHADER_READ` between mvp-build and the vertex shader on the MVP buffer; `COLOR_ATTACHMENT_WRITE→TRANSFER_READ` on the camera color before the blit; swapchain layout transitions) from the resource-usage records carried by each secondary. Each FrameSlot owns the per-frame host-visible staging buffers (`vec4` per entity for pos/rot/scl, **three** dirty bitmasks — one per component, view_proj uniform), three CPU-side pending dirty masks that accumulate per-component drains from `TransformHierarchy::Dirty` until that slot's turn comes, the four per-frame compute descriptor sets (three scatter sets + mvp-build set 1), the four secondaries above + the composing primary. Slots are rebuilt on swapchain recreation, on camera extent change, on camera capacity growth, and on **world entity-capacity growth** — the latter is new and is what the per-component compute scatter pipeline buys: SoT growth invalidates everything that captured SoT handles. **See [`docs/ADR-0002-per-frame-cb-recording.md`](docs/ADR-0002-per-frame-cb-recording.md) for the history (per-frame recording was tried and superseded due to a ~12k→8k FPS regression).** |
...
Current measured frame times (release build, multi-threaded staging, animated `Rotator` scene; see [ADR-0004 §Measurements](docs/ADR-0004-instanced-indirect-draw.md#measurements-post-phase-1) for the full methodology, the pre-/post-Phase-1 comparison, and the `--static-scene` / single-thread A/B columns):

| Cubes     | Frame time | Notes |
|---|---|---|
| 1         | ~0.10 ms  (~10 000 FPS) | GPU floor; single mesh, single instance |
| 1 000     | ~0.20 ms  (~5 000 FPS)  | |
| 10 000    | ~0.69 ms  (~1 450 FPS)  | |
| 100 000   | **~1.01 ms (~990 FPS)** | ~10× speedup from ADR-0004 Phase 1 (was ~9.9 ms) |
| 1 000 000 | **~4.55 ms (~220 FPS)** | New tier; was infeasible before Phase 1 |
...
**The bottleneck has moved to the CPU side.** With per-instance draw cost gone, the per-frame staging-write traffic and `Component::update` cost are exposed: at N=100K, a single-threaded staging walk drops the frame rate from ~990 FPS to ~250 FPS (the rayon path that landed under ADR-0003 is now doing real work), and at N=1M `--static-scene` reduces frame time from ~4.5 ms to ~1.3 ms (the ~3.2 ms delta is per-frame staging-buffer traffic). This unblocks [ADR-0003](docs/ADR-0003-shared-staging-with-compute-sync.md)'s full architectural refactor (single shared staging buffer + timeline-semaphore sync on `COMPUTE_SHADER`-stage end), which is the next major work item: it both saves ~4× VRAM on staging and lets the CPU prep frame N+1 in parallel with the GPU's scatter for frame N. Phase 2 of ADR-0004 (GPU-driven frustum culling) follows after that.
