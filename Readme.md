...
The renderer is **component-driven**: a scene declares drawables by attaching [`MeshRenderer`] components, and geometry flows through the registry's GPU mega buffers. Only async asset loading remains before real meshes (beyond the placeholder) can stream in.
...
**The migration is live (Steps 1 + 2).** Topology comes from `MeshRenderer` components: each pushes `(transform_id, mesh_id)` onto a spawn queue at `init`; the renderer drains it, derives `draws_template[i] = redirect_of(mesh_id)` (the placeholder slot until a loader resolves the asset) + `entity_template[i] = transform_id`, and rebuilds the camera on change. The camera draws from the mega buffers: its scene secondary binds the single mega vertex + mega index buffer **once** and issues one `vkCmdDrawIndexedIndirect` per mesh group, each command's `first_index`/`vertex_offset` (from `GpuMeshStore::slot_geometry`) selecting that mesh's mega-buffer slice. Every `MeshRenderer::new(path)` currently resolves to the placeholder cube (same path dedups to one `MeshId` → one instanced draw group). The per-frame loop also `sync()`s the `GpuMeshStore` and scatters spawns into the device `GPURenderers` buffer — the inputs the planned GPU cull kernel will consume to replace the CPU topology derivation. **Next: async asset loading** (drive `MeshRenderer::new`'s `needs_load` to a background loader that decodes a real `Mesh` and calls `AssetRegistry::resolve`, flipping the redirect so the placeholder swaps to the loaded mesh).
