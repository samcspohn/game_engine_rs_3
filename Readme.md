...
| Camera render targets & matrices | A [`RenderCamera`](crates/engine-render/src/camera.rs) owns the offscreen color image (`R16G16B16A16_SFLOAT`, `COLOR_ATTACHMENT \| TRANSFER_SRC`), the depth image (`D32_SFLOAT`), the **device-local MVP storage buffer** (`STORAGE_BUFFER \| TRANSFER_DST`), the **descriptor set** that points at it, and the pre-recorded **scene secondary** that issues all draws against them. The swapchain image is never used as a color attachment. Each camera carries a `CameraResolution` policy (`MatchSwapchain` today; `Fixed { w, h }` and `ScaleSwapchain { num, denom }` reserved for shadow maps / half-res reflections / editor thumbnails) plus an `allocated_capacity` (matrix-buffer slot count, grown geometrically). Three orthogonal invalidation domains drive rebuilds: **(a) per-camera resolution** (extent change → rebuild attachments + scene secondary; future swapchain-independent cameras survive swapchain resize untouched), **(b) per-camera capacity** (scene topology grows past the current slot count → re-allocate device buffer + descriptor set + scene secondary, geometric ≥ 2× growth so amortized cost is O(1) per added draw), **(c) per-frame-in-flight** (host-mapped staging matrix ring — lives on `FrameSlot`, not the camera). This is the cash-out of the multi-camera roadmap step: the device matrix buffer is allocated **once per camera** rather than once per swapchain image, and the scene secondary references only camera-stable resources so swapchain-independent cameras never re-record on swapchain resize. |
...
| Reusable command buffers | One `MultipleSubmit` primary command buffer **per swapchain image** ("FrameSlot"), composed from two `MultipleSubmit` **secondary** command buffers stratified by invalidation domain: a **scene secondary** (lives on `RenderCamera`; inherits the primary's dynamic-rendering scope; contains viewport, pipeline/descriptor binds, and one `draw_indexed` per `RenderInstance`) and a **blit secondary** (lives on `FrameSlot`; no inheritance; contains the offscreen-color → swapchain-image `vkCmdBlitImage`). The primary records `copy_buffer(staging → camera.device_matrices)`, opens a `begin_rendering(SubpassContents::SecondaryCommandBuffers)` scope on the camera attachments, `execute_commands`s the camera's scene secondary, ends rendering, then `execute_commands`s the slot's blit secondary. Vulkano's auto-sync covers in-CB *and* primary↔secondary transitions, so all barriers (`TRANSFER_WRITE → SHADER_READ`, `COLOR_ATTACHMENT_WRITE → TRANSFER_READ`, swapchain layout transitions) are inferred correctly. Each FrameSlot owns only the host-mapped staging matrix buffer (`HOST_SEQUENTIAL_WRITE`, `TRANSFER_SRC`, sized to match `camera.allocated_capacity()`), the blit secondary, and the composing primary — the device matrix buffer, descriptor set, and scene secondary all live on `RenderCamera` and are shared across slots. Slots are rebuilt on swapchain recreation, on camera extent change, and on capacity growth (the rare third invalidation event). The secondary stratification means swapchain-independent cameras (future) can survive swapchain resize without rebuilding their scene secondary at all — only the per-image blit + primary need re-recording. **See [`docs/ADR-0002-per-frame-cb-recording.md`](docs/ADR-0002-per-frame-cb-recording.md) for the history (per-frame recording was tried and superseded due to a ~12k→8k FPS regression).** |
| Per-frame hot path | (1) Acquire image → wait per-image fence. (2) If `draws_template.len() > camera.allocated_capacity()` (or topology length changed), grow the camera's matrix buffer geometrically and rebuild the affected FrameSlots — this is the rare capacity-invalidation path. (3) Compute per-instance MVPs (`view_proj * model`) into a reusable scratch `Vec<[f32;16]>`. (4) `memcpy` into the slot's mapped staging buffer (first `draw_count` slots; any geometric-grow headroom past that is left untouched). (5) Submit the slot's pre-recorded primary CB. No CB recording, no descriptor-set allocation, no buffer allocation per frame in steady state. |
