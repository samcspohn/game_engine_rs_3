...
**Next:** collapse the per-group draw loop into a single `multiDrawIndexedIndirect` (multiple distinct meshes already render as separate groups today — just use different `.obj` paths), then GPU-driven culling.
