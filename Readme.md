...
| `ComponentStorage<T>` | Per-type dense store backed by `SegStorage<Mutex<T>>` with an `AtomicU32` active-bitset.  Parallel update via the engine's static [`thread_pool`](crates/engine-core/src/util/thread_pool.rs). |
...
| Per-frame hot path | (1) Acquire image → wait per-image fence. (2) If `hierarchy.len() > world.entity_capacity()`, grow the SoT + shared staging buffers, rebuild every camera's mvp-build set 0 + mvp_build_secondary, and rebuild all FrameSlots' primary CBs (per-world axis). (3) If `draws_template.len() > camera.allocated_capacity()` (or topology length changed), grow the camera's MVP buffer geometrically and rebuild the affected FrameSlots (per-camera axis). (4) **Busy-poll `WorldTransformGpu::gpu_signal[0]`** until it reaches `next_signal_expected - 1` (`spin_loop` ×64 → `yield_now` → 100µs `sleep` after ~1ms) — first frame returns immediately because the buffer is pre-zeroed. (5) Drain `TransformHierarchy::Dirty`'s per-component atomic bitmasks (`swap(0, Relaxed)` per word) directly into the **shared** `staging_dirty_{pos,rot,scl}` + `staging_{pos,rot,scl}` via raw SoA accessors (`thread_pool::parallel_for`, no per-entity `Mutex`); write `view_proj` into the shared `staging_view_proj`. (6) Submit the slot's pre-recorded primary CB — plain submit, no extra waits/signals, one `vkQueueSubmit2` + one `vkQueuePresentKHR` per frame. The CB runs scatter (uploads dirty TRS into SoT), the dirty `fill_buffer(0)` clears, the `view_proj` `copy_buffer`, `signal_cs` (atomically increments `gpu_signal`), mvp_build, scene render, blit. Increment `next_signal_expected` after submit so the next frame's poll knows the new target. No CB recording, no descriptor-set allocation, no buffer allocation per frame in steady state. |
...
| `MeshRenderer` | engine-render | ECS component (`HAS_UPDATE = false`) storing only a `MeshId`. `new(path)` resolves the path against `asset::global()` (so the constructor returns a handle, no path stored); `init` pushes `(transform_id, mesh_id)` onto a render-side global spawn queue that the renderer drains to build the draw topology. |
...
**The migration is live (Steps 1 + 2 + async loading).** Topology comes from `MeshRenderer` components: each pushes `(transform_id, mesh_id)` onto a spawn queue at `init`; the renderer drains it, derives `draws_template[i] = redirect_of(mesh_id)` + `entity_template[i] = transform_id`, and rebuilds the camera on change. The camera draws from the mega buffers: its scene secondary binds the single mega vertex + mega index buffer **once** and issues one `vkCmdDrawIndexedIndirect` per mesh group, each command's `first_index`/`vertex_offset` (from `GpuMeshStore::slot_geometry`) selecting that mesh's mega-buffer slice.

**Async loading.** `MeshRenderer::new(path)` resolves the path against the registry (deduped); on the first request of a path it queues an `asset::request_load`, handled by a **dedicated background loader thread** (in `engine-core`, off the fork-join pool) that decodes the file (`.obj` via [`tobj`](https://crates.io/crates/tobj); paths are CWD-relative) into a CPU `Mesh` and calls `AssetRegistry::resolve` (or `fail` → error mesh, logged). The renderer holds only a stable `MeshId`, so completion is a single redirect write — `MeshSlot::PLACEHOLDER` while loading, the real slot once resolved, `MeshSlot::ERROR` on failure — no renderer record is ever patched. Each frame `GpuMeshStore::sync()` uploads any newly-resolved geometry + patches the redirect buffer and reports back; when it reports a change the renderer re-derives topology and rebuilds the camera, so the placeholder instances regroup onto the loaded mesh. The per-frame loop also scatters spawns into the device `GPURenderers` buffer — the input the planned GPU cull kernel will consume to replace the CPU topology derivation.

**Multi-mesh draw.** The scene secondary issues a **single `vkCmdDrawIndexedIndirect`** over the per-group args buffer (`drawCount == number of distinct meshes`) rather than one call per group — distinct `.obj` paths become distinct slots → distinct groups → one fanned-out multi-draw over the shared mega buffers. Per-group `first_instance` (the prefix-sum base) is computed CPU-side in `sort_topology_by_mesh` on topology change; the per-frame visible `instance_count` is written by the cull pass below.

**Next:** GPU-driven culling is **live**. Each frame the per-camera compute pass (the evolved `mvp_build.comp`) does, per draw-slot: compose the model matrix from the SoT TRS, transform the mesh's bounding sphere (from the per-group `group_bounds`, derived from `MeshTableEntry`) into world space, and frustum-test it against `view_proj` (Gribb–Hartmann planes extracted in-shader). Visible instances atomically claim the next slot in their group's MVP region (`base = indirect[g].first_instance`, `local = atomicAdd(indirect[g].instance_count, 1)`) and write the compacted MVP there; culled instances write nothing. The indirect `instance_count` is reset each frame by a `vkCmdCopyBuffer` of a host template (counts pre-zeroed) into the device-local args buffer, recorded just before the cull dispatch in the same secondary — so the multi-draw automatically issues only the visible count per mesh, and the per-group `first_instance` bases stay CPU-maintained (no GPU prefix sum). The vertex shader is unchanged. **Known follow-up:** compaction uses one global `atomicAdd` per visible instance on the group counter, which contends for a single-group scene (the cube benchmark); a workgroup-local aggregation is the planned optimization.
...
**Static thread pool, pinned at startup.** `Window::run` calls `init_pinned_thread_pool` before constructing the winit event loop. It enumerates logical cores via [`core_affinity`](https://crates.io/crates/core_affinity), pins the main thread to core 0, and spawns `(total - 1)` worker threads pinned 1:1 to the remaining cores (workers wrap around the core list if `total` exceeds the physical core count). `ENGINE_NUM_THREADS` (or `RAYON_NUM_THREADS`) sets the **total** participant count including main; it's parsed strictly — bad values panic. Per project rules, every failure path panics: no silent fallback to an unpinned pool.

The pool replaces rayon with a much lighter fork-join scheduler tailored to the engine's per-frame workloads ([`engine_core::util::thread_pool`](crates/engine-core/src/util/thread_pool.rs)). Key properties:

* **Main thread always participates.** A `parallel_for(n_tasks, |task_idx| ...)` call partitions `0..n_tasks` into contiguous slices of size `ceil(n_tasks / (workers + 1))` and assigns one slice per participant (worker `i` gets `[i*chunk, (i+1)*chunk)`, main gets the trailing slice). Main runs its slice inline before barrier-waiting on the workers.
* **Static range partitioning — no work-stealing, no per-task atomic claim, no deques.** Each participant's inner loop is a tight `for i in start..end { f(i) }` with zero synchronisation per task. Workers run the inner loop **inside a monomorphised range thunk** (`unsafe fn invoke_range<F>(data, start, end)`) so `f(i)` inlines fully into the worker's tight loop — the worker pays one indirect call per slice, not one per task, matching the main thread's inline path and unlocking LICM / vectorisation of the closure body. The only atomics per dispatch are one Release `fetch_add` on `epoch` (main publishes the job) and one Release `fetch_add` on `workers_done` per worker (signals slice complete). Each rayon use-site was already a uniform fork-join pattern with hand-tuned chunk sizes; the static pool drops the dispatch overhead rayon was paying for load balancing the engine didn't need.
* **Fast wakeup via `std::thread::Thread::unpark`** (futex on Linux, no mutex on the wake side). Workers spin (~8 192 × `spin_loop` ≈ 30 µs) → `thread::sleep(1ns)` (rounded up to one Linux scheduler tick, ~50 µs, but releases the core so its turbo budget can migrate to main) for the rest of a ~1 ms idle window → `thread::park`. The 1 ms threshold is well above any inter-dispatch gap inside a hot frame at any realistic FPS, so back-to-back parallel-fors during `Scene::update` and the staging walk never park. When the engine has nothing to dispatch (e.g. `--cubes 1` where every `parallel_for` short-circuits on `n_tasks <= 1`) workers go quiet within 1 ms, freeing thermal / turbo budget for the main thread — worth ~40 % at peak FPS on a single-core-bound workload. The main-thread barrier mirrors the same staircase (tight spin → `yield_now` after ~100k spins → 100 µs sleeps after ~10M) so a worker that gets briefly preempted by an OS interrupt can't turn a microsecond hiccup into a multi-ms hitch via runaway main-thread spinning.
* **No nested parallelism.** Calling `parallel_for` from inside a worker panics — recursive descents (e.g. `TransformHierarchy::rotate_children`) walk sequentially within their outer task.
* **No allocation per dispatch.** The job closure is stored as a stack-allocated descriptor (raw pointer + monomorphised dispatch fn), published to workers via an `epoch: AtomicU64` Release/Acquire fence. Main blocks until `workers_done == num_workers` (every worker has signalled completion of its slice), guaranteeing the closure borrow can never dangle.
* **Self-validation via `ENGINE_POOL_VERIFY=1`.** Adds one atomic increment per task invocation and asserts the total at the end of every `parallel_for`; the renderer's staging walk also prints once-per-second tallies of tasks-ran / dirty-words-with-set-bits / entity-bits-processed under the same env var. Off by default — the extra atomic is a measurable cost at high N.
* **Per-call timing breakdown gated on the `pool-timing` Cargo feature.** `parallel_for` always returns a `DispatchTiming { n_tasks, dispatch_ns, main_work_ns, barrier_ns }` struct, but the three `_ns` fields are only populated when `engine-core` is built with `--features pool-timing` — otherwise no `Instant::now()` calls happen in the hot path and the fields are `0`. `engine-render` enables the feature so its per-frame staging walk can attribute latency to dispatch / main-work / barrier; downstream crates that don't need the breakdown can freely discard the return value (no `#[must_use]`).
...
The N≥10K baseline wins came from moving the per-component staging buffers into BAR / ReBAR memory (`MemoryTypeFilter::PREFER_DEVICE | HOST_RANDOM_ACCESS`) so the GPU's scatter compute reads them at full VRAM bandwidth instead of PCIe per cache line. The CPU staging-write loop runs in parallel via the engine's static thread pool (256 dirty-words / 8192 entities per task).
