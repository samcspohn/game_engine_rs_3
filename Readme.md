...
| Camera render targets | Each FrameSlot owns an **offscreen** color image (`R16G16B16A16_SFLOAT`, `COLOR_ATTACHMENT \| TRANSFER_SRC`) and depth image (`D32_SFLOAT`) sized to the current swapchain extent. The scene render writes into these; the swapchain image is never used as a color attachment. This is step 1 of the multi-camera / post-processing roadmap (`todo.txt`): once the camera owns its own attachments, multiple cameras, picture-in-picture, mirrors, and HDR → sRGB tonemapping all become "another pass before the present-blit." |
