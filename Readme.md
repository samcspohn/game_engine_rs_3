...
**Static thread pool, pinned at startup.** `Window::run` calls `init_pinned_thread_pool` before constructing the winit event loop. It enumerates logical cores via [`core_affinity`](https://crates.io/crates/core_affinity), pins the main thread to core 0, and spawns `(total - 1)` worker threads pinned 1:1 to the remaining cores (workers wrap around the core list if `total` exceeds the physical core count, and each worker is also classified as local-DRAM or not via `numa::local_dram_cpus()` against its pinned core, so `Scope::LocalDRAM` dispatches restrict to the memory-die-local subset). `ENGINE_NUM_THREADS` (or `RAYON_NUM_THREADS`) sets the **total** participant count including main; it's parsed strictly — bad values panic. Per project rules, every failure path panics: no silent fallback to an unpinned pool.
...
* **Static range partitioning — no work-stealing, no per-task atomic claim, no deques.** Each participant's inner loop is a tight `for i in start..end { f(i) }` with zero synchronisation per task. The only atomics per dispatch are one Release `fetch_add` on `epoch` (main publishes the job) and one Release `fetch_add` on `workers_done` per worker (signals slice complete). Each rayon use-site was already a uniform fork-join pattern with hand-tuned chunk sizes; the static pool drops the dispatch overhead rayon was paying for load balancing the engine didn't need.
* **Fast wakeup via `std::thread::Thread::unpark`** (futex on Linux, no mutex on the wake side). Workers spin (~4 096 × `spin_loop` ≈ 30 µs) → `yield_now` for the rest of a ~1 ms idle window → `thread::park`. The 1 ms threshold is well above any inter-dispatch gap inside a hot frame at any realistic FPS, so back-to-back parallel-fors during `Scene::update` and the staging walk never park. But when the engine has nothing to dispatch (e.g. `--cubes 1` where every `parallel_for` short-circuits on `n_tasks <= 1`) workers go quiet within 1 ms, freeing thermal / turbo budget for the main thread — worth ~25 % at peak FPS on a single-core-bound workload.
...
* **Single global pool with per-dispatch `Scope` filtering.** One set of OS-worker threads (`init_global_numa(num_workers, on_worker_start, is_local_dram)`); every `parallel_for` dispatch carries a [`Scope`](crates/engine-core/src/util/thread_pool.rs) that names a subset of workers — `Scope::All` (every worker, default — used by `Scene::update`, `par_iter`, `build_frame_slots`) or `Scope::LocalDRAM` (only workers on NUMA nodes with local DRAM, used by the renderer's `host_staging` walk because the memory-bandwidth-bound write is slowed by workers on compute-only NUMA dies that have no local memory controller). Per-worker scope membership is computed once at init by calling the caller-supplied `is_local_dram(worker_idx)` classifier against the NUMA topology, then baked into per-scope worker maps so per-dispatch lookup is O(1). On non-NUMA systems the classifier returns `true` for every worker and `Scope::LocalDRAM` collapses to `Scope::All`. Workers outside the dispatch's scope **observe the epoch advance but do not run the closure and do not signal `done`** — the main thread's barrier waits only for in-scope workers, and out-of-scope parked workers are **not unparked** (they stay quiet until a dispatch in their own scope arrives), so a `Scope::LocalDRAM` walk never burns CPU waking up workers that won't run. This replaces the previous two-pool (compute + staging) design where each pool kept its own set of workers spinning, doubling idle CPU between dispatches. The legacy single-pool API (`init_global(num_workers, on_worker_start)` and `global().parallel_for(...)`) is preserved as a back-compat shim that marks every worker as local DRAM.
