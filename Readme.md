...
| Shaders | GLSL sources live as standalone files under [`crates/engine-render/shaders/`](crates/engine-render/shaders/) (`scene.vert`, `scene.frag`, `scatter.comp`, `mvp_build.comp`) and are compiled to SPIR-V at build time by the `vulkano-shaders` macro via `path:` (each macro registers `cargo:rerun-if-changed` for its source). Splitting them out of `src/shaders.rs` enables editor / GLSL-LSP support, scoped recompiles when iterating on a shader, and reuse by a future SPIR-V on-disk cache. **Three pipelines, two stages each.** Graphics: vertex shader looks up a per-instance MVP from a storage buffer (set 0, binding 0) using `gl_InstanceIndex`; fragment shader does diffuse + ambient shading on the face normal. Compute (`scatter_cs`): one shader, three dispatches per frame — reads a per-frame staging buffer (`vec4` per entity slot) and a per-frame `dirty` bitmask, writes the world-scoped device-local SoT buffer for that component (position / rotation / scale share the descriptor-set layout, only the bound buffers differ). Compute (`mvp_build_cs`): reads the three SoT buffers, indexed via a per-camera `instance → entity` lookup, multiplies by a per-frame `view_proj` (set 1, single mat4 storage buffer) and writes the per-camera MVP buffer the vertex shader will read. |
...
| GPU transform pipeline | A [`WorldTransformGpu`](crates/engine-render/src/transform_gpu.rs) owns the **device-local SoT** ("source of truth") buffers — one per component (`vec4` per entity slot for position / rotation / scale, sized to `entity_capacity`, grown geometrically) — plus the two compute pipelines, **and (post ADR-0003) the single shared per-frame staging triple, the three shared dirty bitmasks, the shared `view_proj` uniform, the three shared scatter descriptor sets, the shared `mvp_build_set1`, the shared scatter compute secondary, and a Vulkan timeline semaphore that gates host writes to all of the above against the previous frame's `COMPUTE_SHADER`-stage completion.** Per frame, in this order, all inside the slot's pre-recorded primary CB: (1) **scatter** ×3 (one dispatch per component) reads `staging_<comp>[i]` + `dirty_<comp>[i]` and writes `sot_<comp>[i]` iff bit `i` is set; (2) **mvp_build** reads SoT pos/rot/scale, indexed via the per-camera `instance → entity` lookup, multiplies by the per-frame `view_proj` and writes the per-camera MVP buffer; (3) the graphics scene secondary draws using that MVP buffer. **Dirty-only sparse upload is live:** each frame the host first calls `host_wait_for_previous_compute()` (timeline-semaphore wait — returns immediately on the first frame because the semaphore's initial value `0` is pre-signaled), then drains `TransformHierarchy::Dirty`'s three per-component `AtomicU32` bitmasks (atomic `swap(0, Relaxed)`) directly into the **shared** staging triple + dirty buffers in one parallel `rayon::par_chunks_mut` walk; the same submit also signals the timeline semaphore at `COMPUTE_SHADER` stage end so the next frame's host wait will gate correctly. Per-component masks mean a pure-rotation frame writes zero pos/scale data on either CPU or GPU. The SoT currently stores **local** TRS — `mvp_build_cs` composes the model matrix directly without parent-chain composition; multi-level hierarchies await a GPU-side global-composition pass. **See [`docs/ADR-0003`](docs/ADR-0003-shared-staging-with-compute-sync.md) for the shared-staging refactor** (replaced 4× per-FrameSlot staging duplication with a single shared instance, saving ~144 MB at N=1M; explicit throughput trade-off at very large N called out in that ADR's measurements section). |
...
| Reusable command buffers | One `MultipleSubmit` primary command buffer **per swapchain image** ("FrameSlot"). Post ADR-0003 `FrameSlot` is minimal — just the per-image `blit_secondary` (camera color → *this* slot's swapchain image) and the composing primary CB; the staging triple, dirty bitmasks, scatter descriptor sets, scatter secondary, mvp_build_set1, and view_proj ring are now **shared** on `WorldTransformGpu`, and `mvp_build_secondaries` are per-camera on `RenderCamera` (one per ring slot, indexed by swapchain image_index). Each frame's `vkQueueSubmit2` carries **two batches**: batch 0 is the shared `world.scatter_primary` (scatter dispatches + dirty fill_buffer clears), batch 1 is the per-image FrameSlot primary (`mvp_build_secondary[image_index]` → `begin_rendering` → `scene_secondary` → `end_rendering` → `blit_secondary`). Batch 0 signals `compute_timeline` at `ALL_TRANSFER` stage end; batch 1 waits on it at `COMPUTE_SHADER` stage — the semaphore signal/wait pair gives mvp_build the SoT memory visibility it needs without any manual `vkCmdPipelineBarrier`. Vulkano's auto-sync handles all the *intra-batch* barriers from each secondary's resource-usage records. Slots are rebuilt on swapchain recreation, on camera extent change, on camera capacity growth, on **world entity-capacity growth**, and on **swapchain image-count change** (which resizes the view_proj ring and re-records the camera's mvp_build_secondaries to match). **See [`docs/ADR-0002-per-frame-cb-recording.md`](docs/ADR-0002-per-frame-cb-recording.md) for the history (per-frame recording was tried and superseded due to a ~12k→8k FPS regression).** |
| Per-frame hot path | (1) Acquire image → wait per-image fence. (2) If `hierarchy.len() > world.entity_capacity()`, grow the SoT + shared staging buffers, rebuild every camera's mvp-build set 0 + mvp_build_secondary, and rebuild all FrameSlots' primary CBs (per-world axis). (3) If `draws_template.len() > camera.allocated_capacity()` (or topology length changed), grow the camera's MVP buffer geometrically and rebuild the affected FrameSlots (per-camera axis). (4) **Host-wait on `WorldTransformGpu::compute_timeline`** for the previous frame's `COMPUTE_SHADER` stage to complete — first frame waits on pre-signaled value 0 (no-op). (5) Drain `TransformHierarchy::Dirty`'s per-component atomic bitmasks (`swap(0, Relaxed)` per word) directly into the **shared** `staging_dirty_{pos,rot,scl}` + `staging_{pos,rot,scl}` via raw SoA accessors (`rayon::par_chunks_mut`, no per-entity `Mutex`); write `view_proj` into the shared mvp-build view_proj buffer. (6) Submit the slot's pre-recorded primary CB; the submit signals the compute timeline at `COMPUTE_SHADER` stage end (next frame's host wait will gate on this) and the per-image fence at end-of-CB — still one `vkQueueSubmit2` + one `vkQueuePresentKHR` per frame. The GPU then runs scatter (uploads dirty TRS into SoT), mvp_build (computes `view_proj * model` per draw), the graphics scene render, and the blit — all inside one CB / one submit. No CB recording, no descriptor-set allocation, no buffer allocation per frame in steady state. |
...
|---|---|---|
| 1         | ~0.13 ms  (~7 800 FPS) | GPU floor; single mesh, single instance. The two extra `vkWaitSemaphores` per frame (timeline + per-image fence) are visible at this trivial workload. |
| 10 000    | ~0.77 ms  (~1 300 FPS) | |
| 100 000   | **~1.27 ms (~785 FPS)** | |
| 1 000 000 | **~5.6 ms (~177 FPS)** | Split-submit (ADR-0003) reclaimed ~33% of the regression vs the intermediate single-submit version (~8.4 ms). Remaining gap to pre-refactor is `mvp_build` running serially with the next frame's submit — fixed properly by ADR-0004 Phase 2 GPU culling. |
...
**The bottleneck is now the GPU's `mvp_build` running serially with the next frame's submit at very large N.** ADR-0003 (shared staging + timeline-semaphore sync, with `view_proj` moved to a per-image-fence-protected ring + a split-submit so scatter and mvp_build live in different batches and the host wait fires at scatter+fill done) **landed**: it saved ~144 MB of VRAM at N=1M (single shared staging triple instead of `MAX_FRAMES_IN_FLIGHT = 4` copies). The remaining throughput cost at N=1M (5.6 ms vs the pre-refactor 4.55 ms) is mvp_build dispatching over `draw_count` invocations every frame. The fix is to make per-frame compute proportional to *visible* entities: ADR-0004 Phase 2 GPU culling (GPU-built indirect args + Hi-Z occlusion). That's now unblocked.
