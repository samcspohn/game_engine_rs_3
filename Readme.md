...
**The renderer is fully GPU-driven (Design B).** There is **no CPU-sorted topology**. Each frame the cull compute pass (`mvp_build.comp`) dispatches one invocation per transform slot and reads `GPURenderers[i] → mesh_id`, `redirect[mesh_id] → slot`, `SoT[i]` (transform), and `mesh_table[slot]` (bounds) directly. It frustum-tests the world bounding sphere (Gribb–Hartmann planes from `view_proj`), and for visible instances atomically claims the next slot in that drawable slot's MVP region (`base = indirect[slot].first_instance`, `local = atomicAdd(indirect[slot].instance_count, 1)`) and writes the compacted MVP. The `instance_count`s are reset each frame by a `vkCmdCopyBuffer` of a host template (counts pre-zeroed) into the device args buffer, recorded just before the cull dispatch. The scene secondary then issues a **single `vkCmdDrawIndexedIndirect`** over `indirect[0..#slots]` against the shared mega buffers.
...
**Static thread pool, pinned at startup.** `Window::run` calls `init_pinned_thread_pool` before constructing the winit event loop. It enumerates logical cores via [`core_affinity`](https://crates.io/crates/core_affinity) and spawns `(total - 1)` worker threads, each pinned to a core (round-robined across NUMA nodes for cache/page locality). The **main thread is deliberately left unpinned** — it spins on the dispatch barrier, and a pinned spinning thread heats its core into throttling while forbidding the OS from migrating it to a cooler one; leaving main free recovers turbo headroom on low-N workloads. `ENGINE_NUM_THREADS` (or `RAYON_NUM_THREADS`) sets the **total** participant count including main; it's parsed strictly — bad values panic. `ENGINE_NO_PIN=1` disables worker pinning entirely (laptops / shared CI / thermal experiments). `ENGINE_AFFINITY_BLOCK=N` widens each worker's pin to an `N`-core migration block. Per project rules, every failure path panics: no silent fallback.

The pool replaces rayon with a much lighter fork-join scheduler tailored to the engine's per-frame workloads ([`engine_core::util::thread_pool`](crates/engine-core/src/util/thread_pool.rs)). It is **node-agnostic**: NUMA-aware containers and per-node dispatch were removed in favour of relying on the OS to migrate pages to whichever node touches them, combined with the pool's deterministic chunk-to-worker assignment (below) which keeps each chunk on the same worker every frame. Key properties:

* **Main thread always participates.** A `parallel_for(n_tasks, |task_idx| ...)` call splits `0..n_tasks` into one contiguous slice per participant (`ceil(n_tasks / active_participants)` tasks each) and seeds participant `p` with the `p`-th slice. Main runs its slice inline (with stealing) before barrier-waiting on the workers.
* **Chunk + tail-steal with deterministic seeding.** Participant `p` is *always* seeded with the `p`-th slice for a given `n_tasks`, so the worker that touched a slice last frame touches it again this frame — cached/paged data stays put without explicit NUMA binding. Each participant drains its own slice via a per-participant [`Cursor`] (a `[start, end)` range packed into a single `AtomicU64`) in `STEAL_GRAIN`-sized claims; when its own slice is exhausted it steals the back half of a neighbour's cursor (rotation order). Owner-claim and steal both CAS the *whole packed word*, so the two ends can never disagree at the boundary (a split `start`/`end` pair double-processes tasks under contention). Stealing recovers throughput when a worker is OS-preempted mid-frame — peers drain the stalled worker's tail and the barrier no longer waits on it. Workers run the inner loop **inside a monomorphised range thunk** so `f(i)` inlines fully (one indirect call per claim, not per task).
* **Fast wakeup via `parking_lot_core`.** Workers spin (`HOT_SPIN_ITERS` × `spin_loop`) → `thread::sleep(1ns)` (~50 µs hrtimer ride-out, releases the core so SMT siblings keep their turbo budget) up to `PARK_AFTER_SPINS` → `parking_lot_core::park` on a sharded key (`ENGINE_PARK_SHARD_SIZE` workers per key, so a wake doesn't serialise on one bucket mutex at high thread counts). Dispatch wakes via one `unpark_all` per shard; workers still in the spin/sleep phase observe the new epoch directly and consume no syscall. The main-thread barrier mirrors a staircase (tight spin → `yield_now` after ~100k spins → 100 µs sleeps after ~10M) so a briefly-preempted worker can't turn a microsecond hiccup into a multi-ms hitch via runaway main-thread spinning.
* **No nested parallelism (yet).** Calling `parallel_for` from inside a worker panics — recursive descents (e.g. `TransformHierarchy::rotate_children`) walk sequentially within their outer task. A job-based `join`/`scope` API with nested parallelism is planned (see [`docs/thread-pool-redesign-plan.md`](docs/thread-pool-redesign-plan.md) Phase 5).
* **No allocation per dispatch beyond the cursor buffer.** The job closure is a stack-allocated descriptor (raw pointer + monomorphised thunk) and a small stack `Vec` of one cursor per participant; both are published to workers via an `epoch: AtomicU64` Release/Acquire fence. Main blocks until `workers_done == active_workers`, guaranteeing the borrow can never dangle.
