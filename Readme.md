...
| Shaders | Compile-time GLSL via `vulkano-shaders`. **Three pipelines, two stages each.** Graphics: vertex shader looks up a per-instance MVP from a storage buffer (set 0, binding 0) using `gl_InstanceIndex`; fragment shader does diffuse + ambient shading on the face normal. Compute (`scatter_cs`): one shader, three dispatches per frame — reads a per-frame staging buffer (`vec4` per entity slot) and a per-frame `dirty` bitmask, writes the world-scoped device-local SoT buffer for that component (position / rotation / scale share the descriptor-set layout, only the bound buffers differ). Compute (`mvp_build_cs`): reads the three SoT buffers, indexed via a per-camera `instance → entity` lookup, multiplies by a per-frame `view_proj` (set 1, single mat4 storage buffer) and writes the per-camera MVP buffer the vertex shader will read. |
...
| GPU transform pipeline | A [`WorldTransformGpu`](crates/engine-render/src/transform_gpu.rs) owns the **device-local SoT** ("source of truth") buffers — one per component (`vec4` per entity slot for position / rotation / scale, sized to `entity_capacity`, grown geometrically) — plus the two compute pipelines. Per frame, in this order, all inside the slot's pre-recorded primary CB: (1) **scatter** ×3 (one dispatch per component) reads `staging_<comp>[i]` + `dirty[i]` and writes `sot_<comp>[i]` iff bit `i` is set; (2) **mvp_build** reads SoT pos/rot/scale, indexed via the per-camera `instance → entity` lookup, multiplies by the per-frame `view_proj` and writes the per-camera MVP buffer; (3) the graphics scene secondary draws using that MVP buffer. The SoT promotion is what makes the device-side transforms readable by future passes (culling, particle emitters, multiple cameras' MVP builds) for free — PCIe traffic stays bounded by what actually changed. The current CPU upload path sets every dirty bit and re-uploads every entity each frame; the dirty-only fast path is structurally enabled (just toggle the bits CPU-side) and will land once `TransformHierarchy::Dirty` is wired to a per-frame-in-flight delta tracker. |
